...
function parseCliArgs(): BatchOptions {
	...
	if (!values.directory) {
		console.error(
			'Usage: npx tsx scripts/icons/batch-process.ts --directory=path/to/svg/directory [--category=category-name] [--filenamePrefix=prefix-] [--skipIndexUpdate]'
		);
		process.exit(1);
	}
...
function updateIconIndex(name: string, category: string): boolean {
	...
	const categoryRegex = new RegExp(
		`(export const iconCategories = \\{[\\s\\S]*?${category}: \\[[\\s\\S]*?)\\]`,
		'm'
	);
	...
	if (categoryMatch[1].match(iconRegex)) {
		...
		const updatedCategoryContent = indexContent.replace(categoryRegex, `$1,\n\t\t'${name}']\n`);
...
function useAdvancedProcessing(filePath: string): string {
	...
	try {
		...
		childProcess.execSync(
			`npx tsx scripts/icons/advanced-svg-processing.ts --file="${filePath}" --output="${tempFilePath}"`,
			{
...
async function main() {
	try {
		...
		const files = fs.readdirSync(directory).filter((file) => file.toLowerCase().endsWith('.svg'));
