...
function parseCliArgs(): ProcessingOptions {
	...
	if (!values.file) {
		console.error(
			'Usage: npx tsx scripts/icons/advanced-svg-processing.ts --file=path/to/svg/file.svg [--output=output-file.svg]'
		);
		process.exit(1);
	}
...
async function main() {
	try {
		...
		console.log(
			`npx tsx scripts/icons/add-icon.ts --name=icon-name --file=${output || 'processed.svg'}`
		);
	} catch (error) {
