...
const categories: CategoryMap = {
  ...
  ui: [
    'search', 'menu', 'settings', 'plus', 'minus', 'check', 'close',
    'notification', 'refresh', 'check-circle', 'mail', 'cart'
  ],
  devices: [
    'mobile', 'tablet', 'desktop', 'phone'
  ],
  cyberpunk: [
    'circuit-board', 'chip', 'cyber-eye', 'matrix', 'data-flow',
    'glitch', 'neural', 'virtual-reality-fill', 'virus-fill', 'vector-three-fill'
  ],
  general: [
...
for (let i = 1; i < iconBlocks.length; i++) {
  ...
  const name = nameMatch[1].trim().toLowerCase().replace(/\s+\(.+\)$/, '');
