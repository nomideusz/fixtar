...
async function fixProductsTimestamps() {
	try {
		...
		if (collection.schema && Array.isArray(collection.schema)) {
			console.log(
				'Fields:',
				collection.schema.map((f) => `${f.name} (${f.type})`)
			);
		...
		while (hasMore) {
			try {
				...
				console.log(
					`📄 Processing page ${page} - ${products.items.length} products with empty timestamps`
				);
				...
				for (const product of products.items) {
					try {
						...
						if (product.created === '') {
							console.log(
								`⏰ Fixing created timestamp for product: ${product.name} (${product.sku})`
							);
						...
						if (product.updated === '') {
							console.log(
								`⏰ Fixing updated timestamp for product: ${product.name} (${product.sku})`
							);
