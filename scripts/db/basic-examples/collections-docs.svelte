...
<p>
	<strong>Collections</strong> represent your application data. Under the hood they are backed by
	plain SQLite tables that are generated automatically with the collection
...
<p>
	Similarly, you can manage your <strong>records</strong> from the Dashboard, with the Web APIs
	using the
	<a href="/docs/how-to-use/">client-side SDKs</a>
...
<img
	src="/images/screenshots/collection-panel.png"
	alt="Collection panel screenshot"
	class="screenshot"
/>
...
<p>
	<strong>View collection</strong> is a read-only collection type where the data is populated from a
	plain SQL <code>SELECT</code> statement, allowing users to perform aggregations or any other
	custom queries in general.
	<br />
...
<p>
	<strong>Auth collection</strong> has everything from the <strong>Base collection</strong> but with some
	additional special fields to help you manage your app users and also providing various authentication
	options.
</p>
<p>
	...
	<br />
	They cannot be renamed or deleted but can be configured using their specific field options. For example
	you can make the user email required or optional.
</p>
<p>
	You can have as many Auth collections as you want (users, managers, staffs, members, clients,
	etc.) each with their own set of fields, separate login and records managing endpoints.
</p>
...
<ul>
	<li>
		...
		<br />
		<em>
			For example, you could attach a "role" <code>select</code> field to your Auth collection with
			the following options: "employee" and "staff". And then in some of your other collections you
			could define the following rule to allow only "staff":
			<br />
	...
	<li>
		...
		<br />
		<em>
			Let's say that you have 2 collections - "posts" base collection and "users" auth collection.
			In your "posts" collection you can create "author"
	...
	<li>
		...
		<em>
			...
			collections have also a special "Manage" API rule that could be used to allow one user (it
			could be even from a different collection) to be able to fully manage the data of another user
			(e.g. changing their email, password, etc.).
		</em>
	</li>
	<li>
		<strong>Mixed</strong>
		<br />
		<em>
			You can build a mixed approach based on your unique use-case. Multiple rules can be grouped
			with parenthesis <code>()</code> and combined with <code>&&</code>
...
<div class="alert alert-info">
	...
	<div class="content">
		<p>
			...
			<strong>non-nullable and uses a zero-default</strong> for their respective type as fallback
			value when missing (empty string for <code>text</code>, 0 for <code>number</code>, etc.).
		</p>
		<p>
...
<div class="accordions">
	...
	<Accordion single title="FileField">
		<div class="content">
			...
			<p>
				PocketBase stores in the database only the file name. The file itself is stored either on
				the local disk or in S3, depending on your application storage settings.
			</p>
			<p>
