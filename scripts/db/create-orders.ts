...
async function createOrdersCollection() {
    try {
      ...
      if (productsExists) {
        ...
        console.warn('Products collection does not exist. Product relation fields will be incomplete.');
      }
      ...
      const result = await pb.collections.create({
        ...
        fields: [
          ...
          },
          {
            ...
            type: 'select',
            required: true,
            values: ['pending', 'processing', 'completed', 'cancelled', 'refunded', 'on_hold', 'failed']
          },
          {
            name: 'items',
            type: 'json',
