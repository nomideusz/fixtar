...
async function updateSyncSessionsCollection() {
  try {
    ...
    if (!authResponse.ok) {
      ...
      if (authResponse.status === 404 || authResponse.status === 400) {
        ...
        if (!altAuthResponse.ok) {
          ...
          throw new Error(`Authentication failed. Make sure you're using PocketBase superuser (admin) credentials.`);
        }
    ...
    console.log('     • status (select, required, options: pending, in_progress, completed, failed)');
...
async function updateCollection(token) {
  try {
    ...
    if (!getResponse.ok) {
      ...
      throw new Error(`Failed to get collection: ${getResponse.status} ${getResponse.statusText} - ${errorText}`);
    }
    ...
    const updatePayload = collection.schema !== undefined 
      ? { schema: newSchema } 
      : { fields: newSchema };
    ...
    if (!updateResponse.ok) {
      ...
      throw new Error(`Failed to update collection: ${updateResponse.status} ${updateResponse.statusText} - ${errorData}`);
    }
