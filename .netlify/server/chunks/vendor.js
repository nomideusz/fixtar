    ...
    if (!value || typeof value !== "object") {
      ...
      if (typeof value[0] === "string") {
        ...
        if (reviver) {
          ...
          hydrating2.add(i);
          hydrated[index] = reviver(hydrate2(i));
          hydrating2.delete(i);
...
async function render_response({
  ...
  if (page_config.csr) {
    const route = manifest._.client.routes?.find((r2) => r2.id === event.route.id) ?? null;
              ...
              if (Pyramid_1) {
                ...
                Pyramid_1($$renderer3, {
                  ...
                  children: ($$renderer4) => {
                    if (Pyramid_2()) {
                      $$renderer4.push("<!--[-->");
                      Pyramid_2()($$renderer4, { data: data_2, form: form2, params: page2.params });
                      $$renderer4.push("<!--]-->");
                    } else {
                      $$renderer4.push("<!--[!-->");
                      $$renderer4.push("<!--]-->");
...
async function get_hooks() {
  ...
  let init2;
  ({ handle, handleFetch, handleError, handleValidationError, init: init2 } = await import("../entries/hooks.server.js"));
  let reroute;
  let transport;
...
function prerender(validate_or_fn, fn_or_options, maybe_options) {
  ...
  const wrapper = (arg) => {
    const promise = (async () => {
      ...
      if (!state2.prerendering && !DEV && !event.isRemoteRequest) {
        try {
