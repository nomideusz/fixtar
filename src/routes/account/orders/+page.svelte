...
<div class="space-y-8">
	{#if hasError}
		...
		<Card class="p-8 text-center">
			<div class="w-16 h-16 bg-danger/10 rounded-2xl flex items-center justify-center mx-auto mb-6">
				<svg class="w-8 h-8 text-danger" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
				</svg>
			</div>
			<h3 class="text-xl font-bold text-neutral-900 mb-2">Wystąpił błąd</h3>
		...
		<section>
			<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
				<Card hover class="group text-center p-6">
					<div class="w-12 h-12 bg-linear-to-br from-brand-100 to-brand-200 rounded-xl flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-200">
						<svg class="w-6 h-6 text-brand-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
						</svg>
					</div>
					<div class="text-2xl font-bold text-brand-600 mb-1">{orderStats().total}</div>
					<div class="text-sm text-neutral-600 font-medium">Łączne zamówienia</div>
				</Card>
				
				<Card hover class="group text-center p-6">
					<div class="w-12 h-12 bg-linear-to-br from-success/10 to-success-light rounded-xl flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-200">
						<svg class="w-6 h-6 text-success" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
						</svg>
					</div>
					<div class="text-2xl font-bold text-success mb-1">{orderStats().delivered}</div>
					<div class="text-sm text-neutral-600 font-medium">Dostarczone</div>
				</Card>
				...
				<Card hover class="group text-center p-6">
					...
					<div class="text-2xl font-bold text-brand-600 mb-1">{orderStats().processing}</div>
					<div class="text-sm text-neutral-600 font-medium">W trakcie</div>
				</Card>
				
				<Card hover class="group text-center p-6">
					<div class="w-12 h-12 bg-linear-to-br from-accent-100 to-accent-200 rounded-xl flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-200">
						<svg class="w-6 h-6 text-accent-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1" />
						</svg>
					</div>
					<div class="text-2xl font-bold text-accent-600 mb-1">{orderStats().totalSpent.toFixed(2)} zł</div>
					<div class="text-sm text-neutral-600 font-medium">Łączne wydatki</div>
				</Card>
			</div>
		</section>
			...
			<Card class="p-6">
				<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
					<div class="flex flex-col sm:flex-row gap-4">
						<!-- Status Filter -->
						<div>
							<label class="block text-sm font-medium text-neutral-700 mb-2">Filtruj po statusie</label>
							<select 
								bind:value={statusFilter}
								class="px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent"
							>
						...
						<!-- Sort -->
						<div>
							<label class="block text-sm font-medium text-neutral-700 mb-2">Sortuj według</label>
							<select 
								bind:value={sortBy}
								class="px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent"
							>
								...
								<div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
									<div class="flex-1 mb-4 lg:mb-0">
										<div class="flex items-center gap-4 mb-3">
											<div>
												<h3 class="text-lg font-bold text-neutral-900 group-hover:text-brand-600 transition-colors">
										...
										<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
											<div>
												...
												<span class="font-semibold text-neutral-900 ml-1">{(order.total || 0).toFixed(2)} zł</span>
											</div>
											<div>
												<span class="text-neutral-500">Metoda płatności:</span>
												<span class="font-semibold text-neutral-900 ml-1">{order.paymentMethod || 'Karta'}</span>
											</div>
											<div>
												<span class="text-neutral-500">Dostawa:</span>
												<span class="font-semibold text-neutral-900 ml-1">{order.shippingMethod || 'Standardowa'}</span>
											</div>
										</div>
									</div>
		...
		<section>
			<Card class="p-8 bg-linear-to-br from-brand-50 to-accent-50 border-2 border-neutral-100">
				<div class="text-center">
					<h3 class="text-xl font-bold text-neutral-900 mb-4">Potrzebujesz pomocy z zamówieniem?</h3>
					<p class="text-neutral-600 mb-6">
					...
					<div class="flex flex-col sm:flex-row gap-4 justify-center">
						...
						<Button href="/help/orders">
							Pomoc z zamówieniami
						</Button>
					</div>
				</div>
			</Card>
