...
<div>
	{#if !userStore.current}
		...
		<div class="mx-auto max-w-2xl">
			<Card class="p-6 text-center sm:p-12">
				<div
					class="mx-auto mb-8 flex h-20 w-20 items-center justify-center rounded-md border border-[--ft-line] bg-[--ft-surface]"
				>
					<UserIcon class="h-10 w-10 text-[--ft-accent]" aria-hidden="true" />
				</div>
				<h2 class="mb-4 text-3xl text-[--ft-text]">Witaj w Swoim Koncie</h2>
	...
	{:else}
		<div class="space-y-12">
			<!-- Welcome Section -->
			<section>
				<Card class="border border-[--ft-line] bg-[--ft-surface] p-4 sm:p-8">
					<div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
						<div class="mb-6 lg:mb-0">
							<div class="mb-4 flex items-center">
								<div
									class="mr-4 flex h-16 w-16 items-center justify-center rounded-md border border-[--ft-line] bg-[--ft-surface]"
								>
							...
							{#if stats.totalOrders > 0}
								<div
									class="border-success/20 bg-success/8 text-success inline-flex items-center rounded-full border px-4 py-2 text-sm"
								>
						...
						<div class="text-center lg:text-right">
							...
							<p class="font-mono text-xl text-[--ft-text]">{memberSince}</p>
						</div>
					</div>
				</Card>
			</section>
			...
			<section>
				<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
					<StatCard value={stats.totalOrders} label="Łączne zamówienia">
						{#snippet icon()}
							<ShoppingBagIcon class="h-8 w-8 text-[--ft-accent]" aria-hidden="true" />
						{/snippet}
					</StatCard>
					...
					<StatCard value={stats.rewardPoints} label="Punkty lojalnościowe">
						{#snippet icon()}
							<StarIcon class="h-8 w-8 text-[--ft-accent]" aria-hidden="true" />
						{/snippet}
					</StatCard>
				</div>
			</section>
			...
			<section>
				<div class="mb-8">
					<h2 class="mb-4 text-2xl text-[--ft-text]">Szybkie akcje</h2>
			...
			<section>
				<div class="mb-8">
					<h2 class="mb-4 text-2xl text-[--ft-text]">Funkcje konta</h2>
					...
					<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
						{#each accountFeatures as feature (feature)}
							<div class="flex items-start">
								<div class="mt-1 shrink-0">
									...
									>
										{#if feature.completed}
											<CheckIcon class="text-success h-4 w-4" aria-hidden="true" />
										{:else}
								...
								<div class="ml-4">
									<h4 class="text-[--ft-text] {feature.completed ? '' : 'opacity-75'}">
										{feature.title}
									</h4>
									<p
										class="mt-1 text-sm text-[--ft-text-muted] {feature.completed
											? ''
											: 'opacity-75'}"
									>
										{feature.description}
			...
			<section>
				<Card class="border border-[--ft-line] bg-[--ft-surface] p-4 sm:p-8">
					<div class="text-center">
						<h3 class="mb-4 text-xl text-[--ft-text]">Potrzebujesz pomocy?</h3>
