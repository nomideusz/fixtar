  ...
  async createTransaction(orderData: {
    ...
    try {
      const response = await axios.post(
        `${this.apiUrl}/transaction/register`,
        data,
        {
          headers: {
...
if (orderData.paymentMethod === 'przelewy24' || orderData.paymentMethod === 'blik' || orderData.paymentMethod === 'card') {
